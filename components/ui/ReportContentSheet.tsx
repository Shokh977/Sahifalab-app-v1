...
import { moderation, apiErrorDetails, type ReportReason } from '../../lib/api'
...
const REASONS: { key: ReportReason; label: string }[] = [
  { key: 'spam',                  label: 'Spam yoki reklama' },
  { key: 'harassment',            label: "Ta'qib yoki haqorat" },
  { key: 'inappropriate_content', label: 'Nomaqbul kontent' },
  { key: 'impersonation',         label: 'Soxta profil' },
  { key: 'other',                 label: 'Boshqa' },
]

export function ReportContentSheet({ visible, targetType, targetId, onClose }: Props) {
  const { c }  = useTheme()
  ...
  async function handleSubmit() {
    ...
    setLoading(true)
    try {
      await moderation.report(targetType, targetId, reason, details.trim() || undefined)
      onClose()
